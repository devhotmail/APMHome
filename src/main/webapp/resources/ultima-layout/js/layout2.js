...
PrimeFaces.widget.Ultima = PrimeFaces.widget.BaseWidget.extend({
  ...
  _bindEvents: function() {
    ...
    this.menuButton.on('click', function(e) {
      var isOpen = !$this.wrapper.hasClass('layout-menu-static-inactive');
      $.cookie('ultima_menu_open', +!isOpen);
  ...
  _checkMenuState: function() {
    function widthChanged(mediaQueryList) {
      $this.toggleMenu(!!mediaQueryList.matches, false);
    }

    var $this = this;
    var mql = window.matchMedia("(min-width: 959px)");
    var persistedMenuClosed = +$.cookie('ultima_menu_open') === 0;

    // always respect menu closed
    if (persistedMenuClosed) {
      this.toggleMenu(false, false);
    } else {
      widthChanged(mql);
    }
    mql.addListener(widthChanged);
  },
  toggleMenu: function(isOpen, animate) {
    var $this = this;
    var transition_bak;
    ...
    if (isOpen) {
      ...
      .addClass('layout-menu-static-inactive');
    }
