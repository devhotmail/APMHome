...
public class AssetSummitUnitTest {
  ...
  private final List<LocalDate> dates = Stream.iterate(LocalDate.now().minusYears(3), now -> now.plusDays(1)).takeUntil(date -> date.isAfter(LocalDate.now())).toJavaList();
  ...
  public void setUp() throws SQLException {
    ...
    assets = db.select(new SQL().SELECT("site_id", "hospital_id", "id", "asset_group", "asset_dept_id").FROM("asset_info").ORDER_BY("id").toString()).getAs(Integer.class, Integer.class, Integer.class, Integer.class, Integer.class).map(t -> Tuple.of(t._1(), t._2(), t._3(), t._4(), t._5(), RandomUtils.nextInt(5) + 1)).cache();
  ...
  public void testFlatMap() {
    Observable.from(dates)
      ...
      .subscribe(t -> db
        ...
        .parameter("revenue", RandomUtils.nextInt(999))
        .parameter("maintenance_cost", RandomUtils.nextInt(99))
        .parameter("deprecation_cost", RandomUtils.nextInt(99))
        .parameter("inject_count", RandomUtils.nextInt(9))
        .parameter("expose_count", RandomUtils.nextInt(9))
        .parameter("film_count", RandomUtils.nextInt(9))
        .parameter("exam_count", RandomUtils.nextInt(9))
        .parameter("exam_duration", RandomUtils.nextInt(999))
        .parameter("down_time", RandomUtils.nextInt(999))
        .parameter("work_order_count", RandomUtils.nextInt(1))
        .parameter("rating", RandomUtils.nextInt(999))
        .parameter("created", t._7).parameter("last_modified", t._7)
        .returnGeneratedKeys()
        .getAs(Integer.class)
        .toBlocking()
        .single());
