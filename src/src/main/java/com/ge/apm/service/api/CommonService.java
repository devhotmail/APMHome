...
public class CommonService {
  ...
  public Map<Integer, Tuple3<Integer, String, String>> findHospitals() {
    return db.select(new SQL().SELECT("id", "name", "alias_name").FROM("site_info").toString())
  ...
  public Map<Integer, String> findDepts(int siteId, int hospitalId) {
    return db.select(new SQL().SELECT_DISTINCT("clinical_dept_id", "clinical_dept_name").FROM("asset_info").WHERE("site_id = :site_id ").WHERE("hospital_id = :hospital_id").toString())
