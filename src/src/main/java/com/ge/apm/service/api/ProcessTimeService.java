...
public class ProcessTimeService {
    ...
    public Observable<Tuple5<String, String, String, String, String>> queryListByType(Integer siteId, Integer hospitalId, LocalDate fromTime, LocalDate toTime, String orderBy, String groupby) {
        ...
        return db.select(SQL_PROCESS_TIME_BY_GROUP.replace(":groupby", groupby.contains("type") ? "ai.asset_group" : "ai.clinical_dept_name,ai.clinical_dept_id")).parameter("site_id", siteId).parameter("hospital_id", hospitalId)
                .parameter("start_time", fromTime).parameter("end_time", toTime).parameter("orderBy", "avg_".concat(orderBy))
    ...
    public Observable<Tuple5<String, String, String, String, String>> queryListByAsset(Integer siteId, Integer hospitalId, LocalDate fromTime, LocalDate toTime, String orderBy, Integer typeId, Integer deptId, Integer offset, Integer limit) {
    ...
    public Observable<Tuple3<String, String, String>> queryGross(Integer siteId, Integer hospitalId, LocalDate fromTime, LocalDate toTime, Integer typeId, Integer deptId) {

        return db.select(SQL_PROCESS_TIME_GROSS.replace(":conditionType", typeId == 0 ? "" : "and ai.asset_group='" + typeId.toString() + "'")
                .replace(":conditionDept", deptId == 0 ? "" : "and ai.clinical_dept_id='" + deptId.toString() + "'"))
                .parameter("site_id", siteId).parameter("hospital_id", hospitalId)
                .parameter("start_time", fromTime).parameter("end_time", toTime)
