...
public class DataMigrateService {
    ...
    public Boolean migrateAttchment(AssetFileAttachment item, String token) {
        
        Map<String,Object> docMap =msService.uploadSingleFileByUrl(token, url_localFileUrl.concat("/").concat(item.getId().toString()),item.getName());
        if(null==docMap){
            return false;
        }
        ...
        blobObject.setObjectSize((Long)docMap.get("objectSize"));
