...
//                     * 则第一年折旧C<1>=X*2/N； 第二年折旧 C<2>=(X-C<1>)*2/N 第三年折旧
//                     * C<3>=(X-C<1>-C<2>)*2/N ··· ··· 最后两年需改为直线法折旧
//                     */
...
//    }
    ...
    public void saveAssetDerpeciation(AssetInfo assetInfo) {
        ...
        if (assetInfo.getPurchasePrice() != null && assetInfo.getSalvageValue() != null
            ...
            assetDepreciation(assetInfo.getPurchasePrice(), assetInfo.getLifecycle(), assetInfo.getDepreciationMethod(), assetInfo.getSalvageValue(), assetInfo.getPurchaseDate(), assetInfo.getId(), assetInfo.getSiteId());
//			assetDepreciationRepository.deleteByAssetIdAndContractType(assetInfo.getId(), -1);
//			
