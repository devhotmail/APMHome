...
public class DmApi {
  ...
  private Observable<Tuple3<Observable<Integer>, Observable<Double>, Observable<Tuple4<Integer, Double, String, Double>>>> averageUsage(Observable<Tuple5<Integer, Double, String, Integer, Double>> predictResult) {
    return
      predictResult.cache().groupBy(k -> k._4).map(g -> Tuple.of(Observable.just(g.getKey()), MathObservable.averageDouble(g.map(v -> v._5)),
        g.map(v -> Tuple.of(v._1, v._2, v._3, v._5)).cache())).cache();
  }
  ...
  private ResponseEntity<Map<String, Object>> serialize(HttpServletRequest request, Map<Integer, String> groups, String groupBy, Observable<Tuple3<Observable<Integer>, Observable<Double>, Observable<Tuple4<Integer, Double, String, Double>>>> items) {
    return ResponseEntity.ok().cacheControl(CacheControl.maxAge(1, TimeUnit.DAYS)).body(new ImmutableMap.Builder<String, Object>()
      ...
      .put("items", items.flatMap(t -> Observable.zip(t._1, t._2, t._3, Tuple::of)).map(v -> new ImmutableMap.Builder<String, Object>()
        .put("id", v._1)
