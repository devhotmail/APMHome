...
public class UserContextService implements Serializable{
    ...
    public static int getInstitutionId(){
        UserAccount userAccount = getCurrentUserAccount();
        if (userAccount!=null) return userAccount.getInstitutionId();
        
        return -1;
    }
    
/*    
    public static SearchFilter getTenantFilter(){
        return new SearchFilter("institutionId", SearchFilter.Operator.EQ, UserContextService.getUserID());
    }
*/

    public static void setInstitutionFilter(List<SearchFilter> filters){
        //filters.add(new SearchFilter("institutionId", SearchFilter.Operator.EQ, UserContextService.getUserID()));
    }
}
